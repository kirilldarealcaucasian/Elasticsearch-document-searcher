...
from common.exceptions import AlreadyExistsError, NotFoundError
...
class ElasticschManager:
    ...
    async def search(
        self, elastic_client: AsyncElasticsearch, text: str, limit: int
    ) -> list[PostS]:
        query = {
            "query": {"match": {"doc.text": text}},
        }
        ...
        try:
            ...
            logger.error(
                msg="failed to search", exc_info=str(e), extra={"text": text}
            )
            raise e
        post_ids: list[str] = []
        try:
            ...
            logger.error(msg="failed to get post_ids", exc_info=str(e))

        return await self._posts_dao.get_posts_by_ids(data=post_ids)

    async def create_index(
        self, elastic_client: AsyncElasticsearch, index_name: str
    ) -> None:
        await elastic_client.indices.delete(
            index=settings.ELASTIC_POSTS_INDEX_NAME, ignore_unavailable=True
        )  # delete an index if exists
        try:
            ...
            if type(e) is (
                ...
                logger.debug(
                    msg="index already exists", extra={"index_name": index_name}
                )
    ...
    def __gen_data(self, data_path: str):
        """
        creates a generator object of documents
        to further be used in bulk add
        """
        try:
        ...
        for document in documents:
            to_add_document = {"id": document["id"], "text": document["text"]}
            yield {
    ...
    async def bulk_add_data(
        ...
        try:
            ...
            logger.info(
                msg="data has been successfully added to elasticsearch index"
            )
        except Exception as e:
            logger.error(
    ...
    async def delete(self, elastic_client: AsyncElasticsearch, post_id: UUID):
        """deletes post from database and index by post id"""
        posts: list = await self._posts_dao.get_posts_by_ids([str(post_id)])

        if len(posts) == 0:
            raise NotFoundError("post you want to delete doesn't exist")

        delete_query = {"match": {"doc.id": str(post_id)}}

        try:
            await elastic_client.delete_by_query(
                index=settings.ELASTIC_POSTS_INDEX_NAME, query=delete_query
            )
        except Exception as e:
            logger.error(
            ...
            raise NotFoundError(detail="post wasn't found")
